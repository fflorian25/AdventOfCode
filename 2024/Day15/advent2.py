...
class Case:
    ...
    def __init__(self, value: str, col: int, row: int):
        ...
        self._isLeftBox = self._value == "["
        self._isRightBox = self._value == "]"
        self._isBox = self._isLeftBox or self._isRightBox

    def fill_leftbox(self):
        self._value = value = "["
        self._isBox = True

    def fill_rightbox(self):
        self._value = value = "]"
        self._isBox = True
...
def read_file(file_path: str):
    ...
    for y, line in enumerate(lines):
        ...
        for x, char in enumerate(line):
            if char == "O":
                map_cases[(2*x, y)] = Case(value="[", col=2*x, row=y)
                map_cases[(2*x+1, y)] = Case(value="]", col=2*x+1, row=y)
            elif char == "@":
                map_cases[(2*x, y)] = Case(value="@", col=2*x, row=y)
                map_cases[(2*x+1, y)] = Case(value=".", col=2*x+1, row=y)
                start_point = (2*x, y)
            else:
                map_cases[(2*x, y)] = Case(value=char, col=2*x, row=y)
                map_cases[(2*x+1, y)] = Case(value=char, col=2*x+1, row=y)
...
if __name__ == "__main__":
    ...
    """
    filename = sys.argv[1] if len(sys.argv) > 1 else 'input_basic3.txt'
