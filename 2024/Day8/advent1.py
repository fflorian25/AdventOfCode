...
def lookForValueSymmetry(antenna, x, y):
    """
    Checks for symmetry relative to the given antenna and computed offsets.
    ...
    Returns:
    tuple: The symmetric position (col + x, row + y).
    """
    # Get the position of the reference antenna
    col, row = antenna.get_col(), antenna.get_row()
    
    # Calculate the symmetric position by applying the offsets
    symmetric_col = col + 2*x
    symmetric_row = row + 2*y
    
    # Return the symmetric position as a tuple
    return (symmetric_col, symmetric_row)
        ...
        for antenna in antennas:  # Loop through each antenna in the list
            for other_antenna in antennas:  # Compare with every other antenna in the list
                if antenna != other_antenna:  # Avoid computing distance with itself
                    ...
                    antinodePosCol.append(lookForValueSymmetry(antenna, x, y))  # Check for symmetry or perform a related operation
        #print(val)
